...
void mwmp_start_recorder(IjkMediaPlayer *mp, const char * recRootPath)
{
    assert(mp);
    pthread_mutex_lock(&mp->mutex);
    mw_start_record(mp->ffplayer, recRootPath);
    pthread_mutex_unlock(&mp->mutex);
}

void mwmp_stop_recorder(IjkMediaPlayer *mp)
{
    assert(mp);
    pthread_mutex_lock(&mp->mutex);
    mw_stop_record(mp->ffplayer);
    pthread_mutex_unlock(&mp->mutex);
}

void mwmp_start_p2p_intercom(IjkMediaPlayer *mp, const char * urlStr, const char * ipStr, int port){
    assert(mp);
    pthread_mutex_lock(&mp->mutex);
    mw_start_p2p_intercom(mp->ffplayer, urlStr, ipStr, port);
    pthread_mutex_unlock(&mp->mutex);
}

void mwmp_stop_p2p_intercom(IjkMediaPlayer *mp){
    assert(mp);
    pthread_mutex_lock(&mp->mutex);
    mw_stop_p2p_intercom(mp->ffplayer);
    pthread_mutex_unlock(&mp->mutex);

}

void mwmp_start_wechat_intercom2(IjkMediaPlayer *mp, int clientSocket){
    assert(mp);
    pthread_mutex_lock(&mp->mutex);
    mw_start_wechat_intercom2(mp->ffplayer, clientSocket);
    pthread_mutex_unlock(&mp->mutex);
}

void mwmp_stop_wechat_intercom2(IjkMediaPlayer *mp, const char *user_id, const char *user_name, const char bemaster, int client_fd){
    assert(mp);
    pthread_mutex_lock(&mp->mutex);
    mw_stop_wechat_intercom2(mp->ffplayer, user_id, user_name,  bemaster, client_fd);
    pthread_mutex_unlock(&mp->mutex);
}

void mwmp_screenshot(IjkMediaPlayer *mp, const char *screenshotRootPath, mw_screenshot_callback callback,void *ffp)
{
    assert(mp);
    pthread_mutex_lock(&mp->mutex);
    mp->ffplayer->screenshot_callback = callback;
    mp->ffplayer->player = ffp;
    mp_screenshot(mp->ffplayer, screenshotRootPath);
    pthread_mutex_unlock(&mp->mutex);
}

void mwmp_screenshot_with_name(IjkMediaPlayer *mp, const char *screenshotRootPath, const char *picName)
{
    assert(mp);
    pthread_mutex_lock(&mp->mutex);
    mp_screenshot_with_name(mp->ffplayer, screenshotRootPath, picName);
    pthread_mutex_unlock(&mp->mutex);
}
