...
@property (nonatomic, copy) void (^screenshotblock)(UIImage *image);
                   ...
                   withOptions:(IJKFFOptions *)options
{
    ...
    if (self) {
        ...
        if (options == nil)
            options = [IJKFFOptions optionsByDefault:NO];
...
void screenshot_callback_(char *filepath, void *ffp) {
    IJKFFMoviePlayerController *player = (__bridge IJKFFMoviePlayerController *)(ffp);
    if (player.screenshotblock) {
        if (filepath == NULL) {
            player.screenshotblock(nil);
        }
        else {
            player.screenshotblock([UIImage imageWithContentsOfFile:[NSString stringWithUTF8String:filepath]]);
        }
    }
}

- (void)screenshot:(void(^)(UIImage *image))block
{
    self.screenshotblock = block;
    mwmp_screenshot(_mediaPlayer, [[NSHomeDirectory() stringByAppendingString:@"/Documents/photos"] UTF8String],screenshot_callback_,(__bridge void *)(self));
}

- (void)screenShot:(NSString *)screenShotRootPath
{
    if (!_mediaPlayer ||  screenShotRootPath == nil)
        return;
    
    //    [self stopHudTimer];
    //ijkmp_pause(_mediaPlayer);
    mwmp_screenshot(_mediaPlayer, [screenShotRootPath UTF8String],screenshot_callback_,(__bridge void *)(self));
}

- (void)screenShot:(NSString *)screenShotRootPath pictureName:(NSString *)pictureName
{
    if (!_mediaPlayer ||  screenShotRootPath == nil || ![pictureName hasSuffix:@".jpg"])
        return;
    mwmp_screenshot_with_name(_mediaPlayer, [screenShotRootPath UTF8String], [pictureName UTF8String]);
}

- (void)start_p2p_intercom:(const char *)urlStr andIp:(const char *) ipStr andPort:(int) port{
    if(!_mediaPlayer)
        return;
    //, const char * urlStr, const char * ipStr, int port)
   mwmp_start_p2p_intercom(_mediaPlayer, urlStr, ipStr, port);
}

- (void)stop_p2p_intercom{
    if(!_mediaPlayer)
        return;
    mwmp_stop_p2p_intercom(_mediaPlayer);
}

- (void)start_p2p_intercom2:(int)clientSocket{
    if(!_mediaPlayer)
        return;
    //, const char * urlStr, const char * ipStr, int port)
    mwmp_start_wechat_intercom2(_mediaPlayer, clientSocket);
}

- (void)stop_p2p_intercom2:(const char *)user_id userName:(const char *)user_name master:(BOOL)beMaster socket:(int)client_fd{
    if(!_mediaPlayer)
        return;
    mwmp_stop_wechat_intercom2(_mediaPlayer, user_id, user_name, beMaster, client_fd);
}

- (NSString *)startRec:(NSString *)recorderRootPath{
    if (!_mediaPlayer &&  recorderRootPath == nil)
        return nil;
    
    //    [self stopHudTimer];
    //ijkmp_pause(_mediaPlayer);
    ijkmp_set_option(_mediaPlayer, IJKMP_OPT_CATEGORY_PLAYER, "local_record_start", "1");
    NSString *fileName = [NSString stringWithFormat:@"%@/%ld.mp4", recorderRootPath, (long)[[NSDate date] timeIntervalSince1970]*1000];
    ijkmp_set_option(_mediaPlayer, IJKMP_OPT_CATEGORY_PLAYER, "local_record_filename", [fileName UTF8String]);
//    mwmp_start_recorder(_mediaPlayer, [recorderRootPath UTF8String]);
    return fileName;
}
...
- (void)stopRec{
    if(!_mediaPlayer)
        return;
    ijkmp_set_option(_mediaPlayer, IJKMP_OPT_CATEGORY_PLAYER, "local_record_start", "0");
//    mwmp_stop_recorder(_mediaPlayer);
}

- (void)startFaceDetect{
    [_glView faceDetector];
}
