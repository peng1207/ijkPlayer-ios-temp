...
@property(nonatomic , assign) NSTimeInterval lastFaceDetectTime;
@end
...
//    NSTimer *timer = [NSTimer scheduledTimerWithTimeInterval:1.0 target:self selector:@selector(faceDetector) userInfo:nil repeats:YES];
//    [[NSRunLoop mainRunLoop] addTimer:timer forMode:NSDefaultRunLoopMode];
}

-(void)faceDetector{
//    while(YES){
    if([[NSDate date] timeIntervalSince1970] - self.lastFaceDetectTime > 1){
//        NSLog(@"detect face start");
        NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
        if(![defaults boolForKey:@"faceDetect"]){
            return;
        }
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            UIImage *screanImage = [self snapshotInternalOnIOS7AndLater];
            
            // 将图像转换为CIImage
            CIImage *faceImage = [CIImage imageWithCGImage:screanImage.CGImage];
            ...
            NSArray *features = [faceDetector featuresInImage:faceImage options:imageOptions];
            ...
            for (CIFaceFeature *faceFeature in features){
                ...
                NSLog(@"detect = %@", NSStringFromCGRect(faceFeature.bounds));
                
                NSLog(@"detect screan bounds = %@", NSStringFromCGRect([[UIScreen mainScreen] bounds]));
                
                if (faceFeature.hasLeftEyePosition) {
                    NSLog(@"detect Left eye %g %g", faceFeature.leftEyePosition.x, faceFeature.leftEyePosition.y);
                }
                if (faceFeature.hasRightEyePosition) {
                    NSLog(@"detect Right eye %g %g", faceFeature.rightEyePosition.x, faceFeature.rightEyePosition.y);
                }
                if (faceFeature.hasMouthPosition) {
                    NSLog(@"detect Mouth %g %g", faceFeature.mouthPosition.x, faceFeature.mouthPosition.y);
                }
            }
            self.lastFaceDetectTime = [[NSDate date] timeIntervalSince1970];
        });
    }
