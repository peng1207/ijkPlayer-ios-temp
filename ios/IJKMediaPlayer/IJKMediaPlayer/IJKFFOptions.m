...
+ (IJKFFOptions *)optionsByDefault:(BOOL)isLive
{
...
//    [options setOptionValue:@"tcp" forKey:@"rtsp_transport"  ofCategory:kIJKFFOptionCategoryFormat];
    if(isLive){
        // Param for living
        [options setPlayerOptionIntValue:500 forKey:@"max_cached_duration"];   // 最大缓存大小是500毫秒，可以依据自己的需求修改
        ...
        [options setFormatOptionIntValue:20000 forKey:@"analyzeduration"];
        NSArray *paths = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES);
        NSString *docDir = [paths objectAtIndex:0];
        NSString *iframePath = [NSString stringWithFormat:@"%@/IFrame.bat", docDir];
        [options setPlayerOptionValue:iframePath forKey:@"iframe-root"];
    }else{
